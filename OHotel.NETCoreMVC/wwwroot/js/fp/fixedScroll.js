...
;(function () {
    let fixedScroll = function ($fixedEl, opts) {
        this.defaults = {
            navEls: '', //nav (注意: navEls 和 hookEls 兩個參數必須成對出現)
            hookEls: '', //nav 要捲動到的對應元素
            hookOffset: 0, //hook 區域頂部偏移量
            offset: 0, //固定元素頂部偏移量
            stickEndEl: '', //固定結束位置的元素
            callback: ''
        };
        $.extend(this, this.defaults, opts);
        this.flag = true;
        this.stickTop = 0; //固定元素的原始位置
        this.init_stickTop = 0; //用於重新計算高度
        this.stickBottom = 9999999; //固定元素的結束位置
